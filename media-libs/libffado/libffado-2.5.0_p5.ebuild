...
if [[ ${PV} == *9999* ]]; then
	...
	SRC_URI="https://ffado.org/files/${PN}-${PV%_p*}.tgz"
	KEYWORDS="~alpha ~amd64 ~arm ~arm64 ~loong ~ppc ~ppc64 ~riscv ~x86"
	S="${WORKDIR}/${PN}-${PV%_p*}"
fi
SRC_URI+=" mirror://debian/pool/main/libf/libffado/${PN}_${PV/_p/-}.debian.tar.xz"


LICENSE="GPL-2 GPL-3"
SLOT="0"
IUSE="debug qt6 test-programs"
...
RDEPEND="
	...
	test-programs? ( media-libs/alsa-lib )
"
DEPEND="${RDEPEND}"
...
myescons() {
	...
	if multilib_is_native_abi; then
		myesconsargs+=(
			BUILD_MIXER=$(usex qt6 true false)
